...
# THIS IS THE POPUP PANEL FOR SHOWING THE USER THAT THEY ARE OUT OF AMMO.
# IT NEEDS TO BE CALLED WHEN THE USER IS OUT OF AMMO AND THE CLIP IS EMPTY.
...
func _process(_delta):
	if check_if_clip_is_empty() and check_active_popups():
		# Set the initial visibility of the popup panel
		show()
		Global.popup_reload = true;
...
# Check if the player's clip is empty
func check_if_clip_is_empty():
	# Check if the player's clip is empty
	# If the player's clip is empty
	if Global.player_current_ammo == 0 or Global.player_reserve_ammo == 0:
		return true
...
func check_active_popups():
	if Global.popup_health or Global.popup_move or Global.popup_shoot:
		return false
	if not Global.popup_reload:
		return true
	return false

# Called when the timer times out
func _on_timer_timeout():
	Global.popup_reload = false;
	hide()  # Hide the popup
